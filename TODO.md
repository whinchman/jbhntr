...
### 6B: Systemd & Docs
- [x] Create deploy/jobhuntr.service systemd unit file (Type=simple, restart on failure, EnvironmentFile for secrets), create .env.example with all required env vars (ANTHROPIC_API_KEY, SERPAPI_KEY, NTFY_TOPIC), create README.md documenting: what jobhuntr does, prerequisites (Go 1.24, Chromium), installation, configuration (config.yaml fields), running (direct and systemd), ntfy setup, usage workflow
